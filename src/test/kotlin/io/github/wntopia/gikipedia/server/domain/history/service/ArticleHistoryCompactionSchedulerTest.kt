...
import io.github.wntopia.gikipedia.server.domain.history.repository.ArticleHistorySegmentKeyView
...
private data class SegmentKeyView(
    private val articleId: Long,
    private val fromRevision: Int,
) : ArticleHistorySegmentKeyView {
    override fun getArticleId() = articleId

    override fun getFromRevision() = fromRevision
}
...
class ArticleHistoryCompactionSchedulerTest {
    ...
    private fun scheduler(enabled: Boolean = true) =
        ArticleHistoryCompactionScheduler(snapshotRepository, segmentRepository, compactionService, enabled)
    ...
    fun compactsOnlyUncompactedClosedSegments() {
        whenever(snapshotRepository.findAllRevisionsOrderByArticleAscRevisionAsc()).thenReturn(
            listOf(
                SnapshotRevisionView(1L, 1),
                SnapshotRevisionView(1L, 4),
                // 7은 아직 다음 스냅샷이 없는 open tail의 마지막 스냅샷 — from으로는 절대 등장하지 않는다.
                SnapshotRevisionView(1L, 7),
            ),
        )
        whenever(segmentRepository.findAllCompactedKeys()).thenReturn(listOf(SegmentKeyView(1L, 1)))
    ...
    fun isolatesPartialFailure() {
        whenever(snapshotRepository.findAllRevisionsOrderByArticleAscRevisionAsc()).thenReturn(
            listOf(
                SnapshotRevisionView(1L, 1),
                SnapshotRevisionView(1L, 4),
                SnapshotRevisionView(2L, 1),
                SnapshotRevisionView(2L, 4),
            ),
        )
        whenever(segmentRepository.findAllCompactedKeys()).thenReturn(emptyList())
